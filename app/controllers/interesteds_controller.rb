class InterestedsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @interested.save
        format.html { redirect_to @interested , notice:"Interesado creado exitosamente" }
  ...
  def update
    respond_to do |format|
      if @interested.update(interested_params)
        format.html { redirect_to @interested , notice:"Modificacion exitosa" }
  ...
  private
    ...
    def interested_params
      params.require(:interested).permit(:nombre, :email, :telefono, :program_id, :area_interest_id, :descripcion, :detail_id, :find_out_id, :why_dcyt_id)
    end
end
