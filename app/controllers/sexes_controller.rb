class SexesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @sex.save
        format.html { redirect_to '/maestricos/prueba?titulo=Sexo&tabla=Sex', notice: 'Sex was successfully created.' }
        format.json { render :show, status: :created, location: @sex }
      else
        format.html { render '/maestricos/prueba?titulo=Sexo&tabla=Sex' }
  ...
  def update
    respond_to do |format|
      if @sex.update(sex_params)
        format.html { redirect_to '/maestricos/prueba?titulo=Sexo&tabla=Sex', notice: 'Sex was successfully updated.' }
        format.json { render :show, status: :ok, location: @sex }
      else
        format.html { redirect_to '/maestricos/edit?tabla=Sex&tabla_id='+@sex.id.to_s }
        format.json { render json: @sex.errors, status: :unprocessable_entity }
      end
    end
  end
  ...
  private
    ...
    def sex_params
      params.require(:sex).permit(:descripcion)
    end
end
